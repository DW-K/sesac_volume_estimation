...
class DensityDatabase():
    ...
    def __init__(self, db_path):
        ...
        if os.path.exists(db_path):
            ...
            self.density_database = pd.read_csv(db_path, usecols=[0, 1], header=None, encoding='euc-kr')
        else:
...
class VolumeEstimator():
    ...
    def __init__(self, arg_init=None):
        ...
        """
        if arg_init is None or arg_init is False:
            # For usage in jupyter notebook 
            print('[*] VolumeEstimator not initialized.')
        else:    
            self.args = self.__parse_args(arg_init)
            ...
            with open(self.args.depth_model_architecture, 'r') as read_file:
                ...
                self.monovideo = model_from_json(model_architecture_json,
                                                 custom_objects=objs)
#             print("----------------------------------------")
#             self.monovideo.summary()
#             model = load_model(self.args.depth_model_weights)
#             model.summary()
#             print("----------------------------------------")
...
if __name__ == '__main__':
    ...
    estimator = VolumeEstimator(arg_init)
